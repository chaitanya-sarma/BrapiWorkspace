...
public class Util {
	...
	public static int getCrops(ArrayList<String> cropList) {
		int status = 0;
		webTarget = client.target("http://bms.icrisat.ac.in:48080/bmsapi/crop/list");
		response = webTarget.request().header("X-Auth-Token", auth.getToken()).get();
		status = response.getStatus();
		if (status == 200) {
			String res = response.readEntity(String.class);
	...
	public static int getCropLocation(String crop, ArrayList<Location> locList) {
		int status = 0;
		webTarget = client.target(baseURI + crop + "/brapi/v1/locations");
		response = webTarget.request().header("X-Auth-Token", auth.getToken()).get();
		status = response.getStatus();
		...
		if (status == 200) {
			...
			for (int i = 0; i < result.size(); i++) {
				...
				loc.setCountryName(ele.get("countryName").toString());
				loc.setLocationDbId(ele.get("locationDbId").toString());
				loc.setLocationType(ele.get("locationType").toString());
				locList.add(loc);
			}
		}
		return status;
